...
loadjs.ready("head", function () {
    ...
    fuserloginadd = currentForm = new ew.Form("fuserloginadd", "add");
    ...
    // Validate form
    fuserloginadd.validate = function () {
    ...
    // Form_CustomValidate
    fuserloginadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fuserloginadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fuserloginadd.lists.userlevels = <?= $Page->userlevels->toClientList($Page) ?>;
    fuserloginadd.lists.id_akses = <?= $Page->id_akses->toClientList($Page) ?>;
    fuserloginadd.lists.status = <?= $Page->status->toClientList($Page) ?>;
    fuserloginadd.lists.dihapus = <?= $Page->dihapus->toClientList($Page) ?>;
    loadjs.done("fuserloginadd");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fuserloginadd" id="fuserloginadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<div class="custom-control custom-checkbox d-inline-block">
    <input type="checkbox" class="custom-control-input<?= $Page->id_akses->isInvalidClass() ?>" data-table="userlogin" data-field="x_id_akses" name="x_id_akses[]" id="x_id_akses_964419" value="1"<?= ConvertToBool($Page->id_akses->CurrentValue) ? " checked" : "" ?><?= $Page->id_akses->editAttributes() ?> aria-describedby="x_id_akses_help">
    <label class="custom-control-label" for="x_id_akses_964419"></label>
</div>
