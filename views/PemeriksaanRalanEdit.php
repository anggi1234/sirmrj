...
// Page object
$PemeriksaanRalanEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var fpemeriksaan_ralanedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    fpemeriksaan_ralanedit = currentForm = new ew.Form("fpemeriksaan_ralanedit", "edit");
    ...
    fpemeriksaan_ralanedit.addFields([
    ...
    // Validate form
    fpemeriksaan_ralanedit.validate = function () {
    ...
    // Form_CustomValidate
    fpemeriksaan_ralanedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpemeriksaan_ralanedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fpemeriksaan_ralanedit.lists.kesadaran = <?= $Page->kesadaran->toClientList($Page) ?>;
    loadjs.done("fpemeriksaan_ralanedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fpemeriksaan_ralanedit" id="fpemeriksaan_ralanedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id_pemeriksaan_ralan->Visible) { // id_pemeriksaan_ralan ?>
    <div id="r_id_pemeriksaan_ralan" class="form-group row">
        <label id="elh_pemeriksaan_ralan_id_pemeriksaan_ralan" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id_pemeriksaan_ralan->caption() ?><?= $Page->id_pemeriksaan_ralan->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id_pemeriksaan_ralan->cellAttributes() ?>>
<span id="el_pemeriksaan_ralan_id_pemeriksaan_ralan">
<span<?= $Page->id_pemeriksaan_ralan->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id_pemeriksaan_ralan->getDisplayValue($Page->id_pemeriksaan_ralan->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="pemeriksaan_ralan" data-field="x_id_pemeriksaan_ralan" data-hidden="1" name="x_id_pemeriksaan_ralan" id="x_id_pemeriksaan_ralan" value="<?= HtmlEncode($Page->id_pemeriksaan_ralan->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
</div><!-- /page* -->
