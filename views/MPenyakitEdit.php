...
loadjs.ready("head", function () {
    ...
    fm_penyakitedit = currentForm = new ew.Form("fm_penyakitedit", "edit");
    ...
    // Validate form
    fm_penyakitedit.validate = function () {
    ...
    // Form_CustomValidate
    fm_penyakitedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fm_penyakitedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fm_penyakitedit.lists.status = <?= $Page->status->toClientList($Page) ?>;
    loadjs.done("fm_penyakitedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fm_penyakitedit" id="fm_penyakitedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" data-table="m_penyakit" data-field="x_kd_penyakit" data-hidden="1" name="o_kd_penyakit" id="o_kd_penyakit" value="<?= HtmlEncode($Page->kd_penyakit->OldValue ?? $Page->kd_penyakit->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
