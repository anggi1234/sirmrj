...
loadjs.ready("head", function () {
    ...
    fcpptedit = currentForm = new ew.Form("fcpptedit", "edit");
    ...
    // Form_CustomValidate
    fcpptedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fcpptedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fcpptedit.lists.profesi = <?= $Page->profesi->toClientList($Page) ?>;
    fcpptedit.lists.verifikasi = <?= $Page->verifikasi->toClientList($Page) ?>;
    loadjs.done("fcpptedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fcpptedit" id="fcpptedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<script>
loadjs.ready(["fcpptedit", "datetimepicker"], function() {
    ew.createDateTimePicker("fcpptedit", "x_tanggal_input", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
...
<?php } ?>
    ...
    <input type="hidden" data-table="cppt" data-field="x_id_cppt" data-hidden="1" name="x_id_cppt" id="x_id_cppt" value="<?= HtmlEncode($Page->id_cppt->CurrentValue) ?>">
<?php if (!$Page->IsModal) { ?>
