...
// Page object
$PasienKunjunganEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var fpasien_kunjunganedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    fpasien_kunjunganedit = currentForm = new ew.Form("fpasien_kunjunganedit", "edit");
    ...
    fpasien_kunjunganedit.addFields([
    ...
    // Validate form
    fpasien_kunjunganedit.validate = function () {
    ...
    // Form_CustomValidate
    fpasien_kunjunganedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpasien_kunjunganedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fpasien_kunjunganedit.lists.no_rkm_medis = <?= $Page->no_rkm_medis->toClientList($Page) ?>;
    fpasien_kunjunganedit.lists.stts = <?= $Page->stts->toClientList($Page) ?>;
    fpasien_kunjunganedit.lists.stts_daftar = <?= $Page->stts_daftar->toClientList($Page) ?>;
    fpasien_kunjunganedit.lists.status_lanjut = <?= $Page->status_lanjut->toClientList($Page) ?>;
    fpasien_kunjunganedit.lists.sttsumur = <?= $Page->sttsumur->toClientList($Page) ?>;
    fpasien_kunjunganedit.lists.status_bayar = <?= $Page->status_bayar->toClientList($Page) ?>;
    fpasien_kunjunganedit.lists.status_poli = <?= $Page->status_poli->toClientList($Page) ?>;
    loadjs.done("fpasien_kunjunganedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fpasien_kunjunganedit" id="fpasien_kunjunganedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id_reg->Visible) { // id_reg ?>
    <div id="r_id_reg" class="form-group row">
        <label id="elh_pasien_kunjungan_id_reg" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id_reg->caption() ?><?= $Page->id_reg->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="hidden" data-table="pasien_kunjungan" data-field="x_id_reg" data-hidden="1" name="x_id_reg" id="x_id_reg" value="<?= HtmlEncode($Page->id_reg->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
