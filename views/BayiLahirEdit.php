...
loadjs.ready("head", function () {
    ...
    fbayi_lahiredit = currentForm = new ew.Form("fbayi_lahiredit", "edit");
    ...
    fbayi_lahiredit.addFields([
        ["id_bayi", [fields.id_bayi.visible && fields.id_bayi.required ? ew.Validators.required(fields.id_bayi.caption) : null], fields.id_bayi.isInvalid],
    ...
    // Validate form
    fbayi_lahiredit.validate = function () {
    ...
    // Form_CustomValidate
    fbayi_lahiredit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fbayi_lahiredit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fbayi_lahiredit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fbayi_lahiredit" id="fbayi_lahiredit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id_bayi->Visible) { // id_bayi ?>
    <div id="r_id_bayi" class="form-group row">
        <label id="elh_bayi_lahir_id_bayi" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id_bayi->caption() ?><?= $Page->id_bayi->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id_bayi->cellAttributes() ?>>
<span id="el_bayi_lahir_id_bayi">
<span<?= $Page->id_bayi->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id_bayi->getDisplayValue($Page->id_bayi->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="bayi_lahir" data-field="x_id_bayi" data-hidden="1" name="x_id_bayi" id="x_id_bayi" value="<?= HtmlEncode($Page->id_bayi->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fbayi_lahiredit", "datetimepicker"], function() {
    ew.createDateTimePicker("fbayi_lahiredit", "x_tanggal_lahir", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
...
<script>
loadjs.ready(["fbayi_lahiredit", "timepicker"], function() {
    ew.createTimePicker("fbayi_lahiredit", "x_jam_lahir", {"timeFormat":"H:i:s","step":15});
});
</script>
<?php } ?>
</span>
