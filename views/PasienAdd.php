...
loadjs.ready("head", function () {
    ...
    fpasienadd.lists.jk = <?= $Page->jk->toClientList($Page) ?>;
    fpasienadd.lists.gol_darah = <?= $Page->gol_darah->toClientList($Page) ?>;
    fpasienadd.lists.stts_nikah = <?= $Page->stts_nikah->toClientList($Page) ?>;
    fpasienadd.lists.agama = <?= $Page->agama->toClientList($Page) ?>;
    fpasienadd.lists.pnd = <?= $Page->pnd->toClientList($Page) ?>;
    fpasienadd.lists.kd_pj = <?= $Page->kd_pj->toClientList($Page) ?>;
    fpasienadd.lists.kd_kel = <?= $Page->kd_kel->toClientList($Page) ?>;
    fpasienadd.lists.kd_kec = <?= $Page->kd_kec->toClientList($Page) ?>;
    fpasienadd.lists.kd_kab = <?= $Page->kd_kab->toClientList($Page) ?>;
    fpasienadd.lists.kd_prop = <?= $Page->kd_prop->toClientList($Page) ?>;
    fpasienadd.lists.suku_bangsa = <?= $Page->suku_bangsa->toClientList($Page) ?>;
    fpasienadd.lists.bahasa_pasien = <?= $Page->bahasa_pasien->toClientList($Page) ?>;
    fpasienadd.lists.cacat_fisik = <?= $Page->cacat_fisik->toClientList($Page) ?>;
...
loadjs.ready(["fpasienadd", "datetimepicker"], function() {
    ew.createDateTimePicker("fpasienadd", "x_tgl_lahir", {"ignoreReadonly":true,"useCurrent":false,"format":7});
});
</script>
<?php } ?>
</span>
...
<span id="el_pasien_kd_prop">
    <select
        ...
        data-select2-id="pasien_x_kd_prop"
        data-table="pasien"
...
<script>
loadjs.ready("head", function() {
    var el = document.querySelector("select[data-select2-id='pasien_x_kd_prop']"),
        options = { name: "x_kd_prop", selectId: "pasien_x_kd_prop", language: ew.LANGUAGE_ID, dir: ew.IS_RTL ? "rtl" : "ltr" };
    options.dropdownParent = $(el).closest("#ew-modal-dialog, #ew-add-opt-dialog")[0];
    Object.assign(options, ew.vars.tables.pasien.fields.kd_prop.selectOptions);
    ew.createSelect(options);
});
</script>
</span>
