...
loadjs.ready("head", function () {
    ...
    fpoliklinikadd = currentForm = new ew.Form("fpoliklinikadd", "add");
    ...
    // Validate form
    fpoliklinikadd.validate = function () {
    ...
    // Form_CustomValidate
    fpoliklinikadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpoliklinikadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fpoliklinikadd.lists.status = <?= $Page->status->toClientList($Page) ?>;
    loadjs.done("fpoliklinikadd");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fpoliklinikadd" id="fpoliklinikadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<div class="custom-control custom-checkbox d-inline-block">
    <input type="checkbox" class="custom-control-input<?= $Page->status->isInvalidClass() ?>" data-table="poliklinik" data-field="x_status" name="x_status[]" id="x_status_636413" value="1"<?= ConvertToBool($Page->status->CurrentValue) ? " checked" : "" ?><?= $Page->status->editAttributes() ?> aria-describedby="x_status_help">
    <label class="custom-control-label" for="x_status_636413"></label>
</div>
