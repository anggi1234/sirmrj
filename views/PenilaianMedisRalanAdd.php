...
// Page object
$PenilaianMedisRalanAdd = &$Page;
?>
<script>
var currentForm, currentPageID;
var fpenilaian_medis_ralanadd;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "add";
    fpenilaian_medis_ralanadd = currentForm = new ew.Form("fpenilaian_medis_ralanadd", "add");
    ...
    fpenilaian_medis_ralanadd.addFields([
    ...
    // Set invalid fields
    $(function() {
        var f = fpenilaian_medis_ralanadd,
            fobj = f.getForm(),
            $fobj = $(fobj),
    ...
    // Validate form
    fpenilaian_medis_ralanadd.validate = function () {
    ...
    // Form_CustomValidate
    fpenilaian_medis_ralanadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpenilaian_medis_ralanadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fpenilaian_medis_ralanadd.lists.anamnesis = <?= $Page->anamnesis->toClientList($Page) ?>;
    fpenilaian_medis_ralanadd.lists.keadaan = <?= $Page->keadaan->toClientList($Page) ?>;
    fpenilaian_medis_ralanadd.lists.kesadaran = <?= $Page->kesadaran->toClientList($Page) ?>;
    loadjs.done("fpenilaian_medis_ralanadd");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fpenilaian_medis_ralanadd" id="fpenilaian_medis_ralanadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php } else { ?>
<span id="el_penilaian_medis_ralan_no_rawat">
<input type="<?= $Page->no_rawat->getInputTextType() ?>" data-table="penilaian_medis_ralan" data-field="x_no_rawat" name="x_no_rawat" id="x_no_rawat" size="30" maxlength="17" placeholder="<?= HtmlEncode($Page->no_rawat->getPlaceHolder()) ?>" value="<?= $Page->no_rawat->EditValue ?>"<?= $Page->no_rawat->editAttributes() ?> aria-describedby="x_no_rawat_help">
...
<input type="<?= $Page->kd_dokter->getInputTextType() ?>" data-table="penilaian_medis_ralan" data-field="x_kd_dokter" name="x_kd_dokter" id="x_kd_dokter" size="30" maxlength="20" placeholder="<?= HtmlEncode($Page->kd_dokter->getPlaceHolder()) ?>" value="<?= $Page->kd_dokter->EditValue ?>"<?= $Page->kd_dokter->editAttributes() ?> aria-describedby="x_kd_dokter_help">
