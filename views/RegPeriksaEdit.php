...
var freg_periksaedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    freg_periksaedit = currentForm = new ew.Form("freg_periksaedit", "edit");
    ...
    // Validate form
    freg_periksaedit.validate = function () {
    ...
    // Form_CustomValidate
    freg_periksaedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    freg_periksaedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    freg_periksaedit.lists.no_rkm_medis = <?= $Page->no_rkm_medis->toClientList($Page) ?>;
    freg_periksaedit.lists.stts = <?= $Page->stts->toClientList($Page) ?>;
    freg_periksaedit.lists.stts_daftar = <?= $Page->stts_daftar->toClientList($Page) ?>;
    freg_periksaedit.lists.status_lanjut = <?= $Page->status_lanjut->toClientList($Page) ?>;
    freg_periksaedit.lists.sttsumur = <?= $Page->sttsumur->toClientList($Page) ?>;
    freg_periksaedit.lists.status_bayar = <?= $Page->status_bayar->toClientList($Page) ?>;
    freg_periksaedit.lists.status_poli = <?= $Page->status_poli->toClientList($Page) ?>;
    loadjs.done("freg_periksaedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="freg_periksaedit" id="freg_periksaedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" data-table="reg_periksa" data-field="x_no_rawat" data-hidden="1" name="o_no_rawat" id="o_no_rawat" value="<?= HtmlEncode($Page->no_rawat->OldValue ?? $Page->no_rawat->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["freg_periksaedit"], function() {
    freg_periksaedit.createAutoSuggest(Object.assign({"id":"x_no_rkm_medis","forceSelect":false}, ew.vars.tables.reg_periksa.fields.no_rkm_medis.autoSuggestOptions));
});
</script>
