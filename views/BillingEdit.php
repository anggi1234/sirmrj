...
loadjs.ready("head", function () {
    ...
    fbillingedit = currentForm = new ew.Form("fbillingedit", "edit");
    ...
    // Validate form
    fbillingedit.validate = function () {
    ...
    // Form_CustomValidate
    fbillingedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fbillingedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fbillingedit.lists.status = <?= $Page->status->toClientList($Page) ?>;
    loadjs.done("fbillingedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fbillingedit" id="fbillingedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<script>
loadjs.ready(["fbillingedit", "datetimepicker"], function() {
    ew.createDateTimePicker("fbillingedit", "x_tgl_byr", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
...
<?php } ?>
    ...
    <input type="hidden" data-table="billing" data-field="x_id_billing" data-hidden="1" name="x_id_billing" id="x_id_billing" value="<?= HtmlEncode($Page->id_billing->CurrentValue) ?>">
<?php if (!$Page->IsModal) { ?>
