...
loadjs.ready("head", function () {
    ...
    fvigdedit = currentForm = new ew.Form("fvigdedit", "edit");
    ...
    // Form_CustomValidate
    fvigdedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fvigdedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fvigdedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fvigdedit" id="fvigdedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id_reg->Visible) { // id_reg ?>
    <div id="r_id_reg" class="form-group row">
        <label id="elh_vigd_id_reg" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id_reg->caption() ?><?= $Page->id_reg->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="hidden" data-table="vigd" data-field="x_id_reg" data-hidden="1" name="x_id_reg" id="x_id_reg" value="<?= HtmlEncode($Page->id_reg->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<input type="hidden" data-table="vigd" data-field="x_kd_dokter" data-hidden="1" name="x_kd_dokter" id="x_kd_dokter" value="<?= HtmlEncode($Page->kd_dokter->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<input type="hidden" data-table="vigd" data-field="x_no_rkm_medis" data-hidden="1" name="x_no_rkm_medis" id="x_no_rkm_medis" value="<?= HtmlEncode($Page->no_rkm_medis->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<input type="hidden" data-table="vigd" data-field="x_kd_poli" data-hidden="1" name="x_kd_poli" id="x_kd_poli" value="<?= HtmlEncode($Page->kd_poli->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<input type="hidden" data-table="vigd" data-field="x_stts" data-hidden="1" name="x_stts" id="x_stts" value="<?= HtmlEncode($Page->stts->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<input type="hidden" data-table="vigd" data-field="x_umurdaftar" data-hidden="1" name="x_umurdaftar" id="x_umurdaftar" value="<?= HtmlEncode($Page->umurdaftar->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
