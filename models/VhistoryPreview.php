 ...
 * Page class
 */
class VhistoryPreview extends Vhistory
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "VhistoryPreview";
    ...
    protected function masterKeyUrl()
    {
        ...
        if ($masterTblVar == "vrajal") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=vrajal&" . GetForeignKeyUrl("fk_no_rkm_medis", $this->no_rkm_medis->QueryStringValue) . "";
        }
        if ($masterTblVar == "vigd") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=vigd&" . GetForeignKeyUrl("fk_no_rkm_medis", $this->no_rkm_medis->QueryStringValue) . "";
        }
        return $url;
    }
    ...
    protected function setupForeignKeysFromFilter($f)
    {
        $masterTblVar = Get("t", "");
        if ($masterTblVar == "vrajal") {
            $find = "pasien_kunjungan.no_rkm_medis=";
            $x = strpos($f, $find);
            if ($x !== false) {
                $x += strlen($find);
                $val = substr($f, $x);
                $val = $this->unquoteValue($val, "DB");
                 $this->no_rkm_medis->setQueryStringValue($val);
            }
        }
        if ($masterTblVar == "vigd") {
            $find = "pasien_kunjungan.no_rkm_medis=";
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("VhistoryList"), "", $this->TableVar, true);
        $pageId = "preview";
