 ...
 * Page class
 */
class PenilaianAwalKeperawatanRalanEdit extends PenilaianAwalKeperawatanRalan
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PenilaianAwalKeperawatanRalanEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("id_penilaian_awal_keperawatan") ?? Key(0) ?? Route(2)) !== null) {
                $this->id_penilaian_awal_keperawatan->setQueryStringValue($keyValue);
                $this->id_penilaian_awal_keperawatan->setOldValue($this->id_penilaian_awal_keperawatan->QueryStringValue);
                ...
                $this->id_penilaian_awal_keperawatan->setOldValue($this->id_penilaian_awal_keperawatan->FormValue);
            } else {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->id_penilaian_awal_keperawatan->IsDetailKey) {
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // alat_bantu
            $this->alat_bantu->setDbValueDef($rsnew, $this->alat_bantu->CurrentValue, "", $this->alat_bantu->ReadOnly);

            // ket_bantu
            $this->ket_bantu->setDbValueDef($rsnew, $this->ket_bantu->CurrentValue, "", $this->ket_bantu->ReadOnly);

            // prothesa
            $this->prothesa->setDbValueDef($rsnew, $this->prothesa->CurrentValue, "", $this->prothesa->ReadOnly);

            // ket_pro
            $this->ket_pro->setDbValueDef($rsnew, $this->ket_pro->CurrentValue, "", $this->ket_pro->ReadOnly);

            // adl
            $this->adl->setDbValueDef($rsnew, $this->adl->CurrentValue, "", $this->adl->ReadOnly);

            // status_psiko
            $this->status_psiko->setDbValueDef($rsnew, $this->status_psiko->CurrentValue, "", $this->status_psiko->ReadOnly);

            // ket_psiko
            $this->ket_psiko->setDbValueDef($rsnew, $this->ket_psiko->CurrentValue, "", $this->ket_psiko->ReadOnly);

            // hub_keluarga
            $this->hub_keluarga->setDbValueDef($rsnew, $this->hub_keluarga->CurrentValue, "", $this->hub_keluarga->ReadOnly);

            // tinggal_dengan
            $this->tinggal_dengan->setDbValueDef($rsnew, $this->tinggal_dengan->CurrentValue, "", $this->tinggal_dengan->ReadOnly);

            // ket_tinggal
            $this->ket_tinggal->setDbValueDef($rsnew, $this->ket_tinggal->CurrentValue, "", $this->ket_tinggal->ReadOnly);

            // ekonomi
            $this->ekonomi->setDbValueDef($rsnew, $this->ekonomi->CurrentValue, "", $this->ekonomi->ReadOnly);

            // budaya
            $this->budaya->setDbValueDef($rsnew, $this->budaya->CurrentValue, "", $this->budaya->ReadOnly);

            // ket_budaya
            $this->ket_budaya->setDbValueDef($rsnew, $this->ket_budaya->CurrentValue, "", $this->ket_budaya->ReadOnly);

            // edukasi
            $this->edukasi->setDbValueDef($rsnew, $this->edukasi->CurrentValue, "", $this->edukasi->ReadOnly);

            // ket_edukasi
            $this->ket_edukasi->setDbValueDef($rsnew, $this->ket_edukasi->CurrentValue, "", $this->ket_edukasi->ReadOnly);

            // berjalan_a
            $this->berjalan_a->setDbValueDef($rsnew, $this->berjalan_a->CurrentValue, "", $this->berjalan_a->ReadOnly);

            // berjalan_b
            $this->berjalan_b->setDbValueDef($rsnew, $this->berjalan_b->CurrentValue, "", $this->berjalan_b->ReadOnly);

            // berjalan_c
            $this->berjalan_c->setDbValueDef($rsnew, $this->berjalan_c->CurrentValue, "", $this->berjalan_c->ReadOnly);
