 ...
 * Page class
 */
class PemeriksaanRalanView extends PemeriksaanRalan
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PemeriksaanRalanView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("id_pemeriksaan_ralan") ?? Route("id_pemeriksaan_ralan")) !== null) {
            $this->RecKey["id_pemeriksaan_ralan"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("id_pemeriksaan_ralan") ?? Route("id_pemeriksaan_ralan")) !== null) {
                ...
                $this->RecKey["id_pemeriksaan_ralan"] = $this->id_pemeriksaan_ralan->QueryStringValue;
            } elseif (Post("id_pemeriksaan_ralan") !== null) {
                $this->id_pemeriksaan_ralan->setFormValue(Post("id_pemeriksaan_ralan"));
                $this->RecKey["id_pemeriksaan_ralan"] = $this->id_pemeriksaan_ralan->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->id_pemeriksaan_ralan->setQueryStringValue($keyValue);
                $this->RecKey["id_pemeriksaan_ralan"] = $this->id_pemeriksaan_ralan->QueryStringValue;
            } else {
                $returnUrl = "PemeriksaanRalanList"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "PemeriksaanRalanList"; // No matching record, return to list
                    }
                    break;
            }
        } else {
            $returnUrl = "PemeriksaanRalanList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
