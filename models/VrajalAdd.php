 ...
 * Page class
 */
class VrajalAdd extends Vrajal
{
    use MessagesTrait;
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("VrajalList"); // No matching record, return to list
                    return;
                }
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "VrajalList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "VrajalView") {
    ...
    protected function validateForm()
    {
        ...
        if (in_array("cppt", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->validateGridForm();
        }
        $detailPage = Container("BillingGrid");
        if (in_array("billing", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->validateGridForm();
        }
        $detailPage = Container("KonsulGrid");
        if (in_array("konsul", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->validateGridForm();
        }
        ...
        if (in_array("resep_dokter", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->validateGridForm();
        }
        $detailPage = Container("PrmrjGrid");
        if (in_array("prmrj", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->validateGridForm();
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($addRow) {
            ...
            if (in_array("cppt", $detailTblVar) && $detailPage->DetailAdd) {
                $detailPage->no_reg->setSessionValue($this->id_reg->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "cppt"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->no_reg->setSessionValue(""); // Clear master key if insert failed
            ...
            if (in_array("billing", $detailTblVar) && $detailPage->DetailAdd) {
                $detailPage->no_reg->setSessionValue($this->id_reg->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "billing"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->no_reg->setSessionValue(""); // Clear master key if insert failed
            ...
            if (in_array("konsul", $detailTblVar) && $detailPage->DetailAdd) {
                $detailPage->no_reg->setSessionValue($this->id_reg->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "konsul"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->no_reg->setSessionValue(""); // Clear master key if insert failed
            ...
            if (in_array("resep_dokter", $detailTblVar) && $detailPage->DetailAdd) {
                $detailPage->no_reg->setSessionValue($this->id_reg->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "resep_dokter"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->no_reg->setSessionValue(""); // Clear master key if insert failed
            ...
            if (in_array("prmrj", $detailTblVar) && $detailPage->DetailAdd) {
                ...
                $Security->loadCurrentUserLevel($this->ProjectID . "prmrj"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
