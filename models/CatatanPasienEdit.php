 ...
 * Page class
 */
class CatatanPasienEdit extends CatatanPasien
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "CatatanPasienEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("no_rkm_medis") ?? Key(0) ?? Route(2)) !== null) {
                $this->no_rkm_medis->setQueryStringValue($keyValue);
                $this->no_rkm_medis->setOldValue($this->no_rkm_medis->QueryStringValue);
            } elseif (Post("no_rkm_medis") !== null) {
                $this->no_rkm_medis->setFormValue(Post("no_rkm_medis"));
                $this->no_rkm_medis->setOldValue($this->no_rkm_medis->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("no_rkm_medis") ?? Route("no_rkm_medis")) !== null) {
                    $this->no_rkm_medis->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$loaded) { // Load record based on key
                    ...
                    $this->terminate("CatatanPasienList"); // No matching record, return to list
                    return;
                }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "CatatanPasienList") {
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // no_rkm_medis
            $this->no_rkm_medis->setDbValueDef($rsnew, $this->no_rkm_medis->CurrentValue, "", $this->no_rkm_medis->ReadOnly);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("CatatanPasienList"), "", $this->TableVar, true);
        $pageId = "edit";
