...
class UserloginAdd extends Userlogin
{
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->userlevels->CurrentValue = null;
        $this->userlevels->OldValue = $this->userlevels->CurrentValue;
        $this->nama->CurrentValue = null;
        $this->nama->OldValue = $this->nama->CurrentValue;
        $this->id_akses->CurrentValue = null;
        $this->id_akses->OldValue = $this->id_akses->CurrentValue;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'userlevels' first before field var 'x_userlevels'
        $val = $CurrentForm->hasValue("userlevels") ? $CurrentForm->getValue("userlevels") : $CurrentForm->getValue("x_userlevels");
        if (!$this->userlevels->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->userlevels->Visible = false; // Disable update for API request
            } else {
                $this->userlevels->setFormValue($val);
            }
        }
        ...
        // Check field name 'id_akses' first before field var 'x_id_akses'
        $val = $CurrentForm->hasValue("id_akses") ? $CurrentForm->getValue("id_akses") : $CurrentForm->getValue("x_id_akses");
        if (!$this->id_akses->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->id_akses->Visible = false; // Disable update for API request
            } else {
                $this->id_akses->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->userlevels->CurrentValue = $this->userlevels->FormValue;
        $this->nama->CurrentValue = $this->nama->FormValue;
        $this->id_akses->CurrentValue = $this->id_akses->FormValue;
    ...
    protected function newRow()
    {
        ...
        $row['userlevels'] = $this->userlevels->CurrentValue;
        $row['nama'] = $this->nama->CurrentValue;
        $row['id_akses'] = $this->id_akses->CurrentValue;
    ...
    protected function validateForm()
    {
        ...
        if ($this->userlevels->Required) {
            if (!$this->userlevels->IsDetailKey && EmptyValue($this->userlevels->FormValue)) {
                $this->userlevels->addErrorMessage(str_replace("%s", $this->userlevels->caption(), $this->userlevels->RequiredErrorMessage));
            }
        }
        ...
        if ($this->id_akses->Required) {
            if ($this->id_akses->FormValue == "") {
                $this->id_akses->addErrorMessage(str_replace("%s", $this->id_akses->caption(), $this->id_akses->RequiredErrorMessage));
            }
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // userlevels
        if ($Security->canAdmin()) { // System admin
            $this->userlevels->setDbValueDef($rsnew, $this->userlevels->CurrentValue, null, false);
        }
        ...
        // id_akses
        $tmpBool = $this->id_akses->CurrentValue;
        if ($tmpBool != "1" && $tmpBool != "0") {
            $tmpBool = !empty($tmpBool) ? "1" : "0";
        }
        $this->id_akses->setDbValueDef($rsnew, $tmpBool, null, false);
