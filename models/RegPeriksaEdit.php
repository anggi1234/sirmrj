 ...
 * Page class
 */
class RegPeriksaEdit extends RegPeriksa
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "RegPeriksaEdit";
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$loaded) { // Load record based on key
                    ...
                    $this->terminate("RegPeriksaList"); // No matching record, return to list
                    return;
                }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "RegPeriksaList") {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->tgl_registrasi->IsDetailKey) {
            ...
            $this->tgl_registrasi->CurrentValue = UnFormatDateTime($this->tgl_registrasi->CurrentValue, 0);
        }
        ...
        if (!$this->id_reg->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->id_reg->Visible = false; // Disable update for API request
            } else {
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->no_rkm_medis->EditValue = HtmlEncode($this->no_rkm_medis->CurrentValue);
            $curVal = trim(strval($this->no_rkm_medis->CurrentValue));
            if ($curVal != "") {
            ...
            $this->id_reg->EditValue = HtmlEncode($this->id_reg->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if (!CheckInteger($this->id_reg->FormValue)) {
            $this->id_reg->addErrorMessage($this->id_reg->getErrorMessage(false));
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // kd_dokter
            $this->kd_dokter->setDbValueDef($rsnew, $this->kd_dokter->CurrentValue, null, $this->kd_dokter->ReadOnly);

            // no_rkm_medis
            $this->no_rkm_medis->setDbValueDef($rsnew, $this->no_rkm_medis->CurrentValue, null, $this->no_rkm_medis->ReadOnly);

            // kd_poli
            $this->kd_poli->setDbValueDef($rsnew, $this->kd_poli->CurrentValue, null, $this->kd_poli->ReadOnly);

            // p_jawab
            $this->p_jawab->setDbValueDef($rsnew, $this->p_jawab->CurrentValue, null, $this->p_jawab->ReadOnly);

            // almt_pj
            $this->almt_pj->setDbValueDef($rsnew, $this->almt_pj->CurrentValue, null, $this->almt_pj->ReadOnly);

            // hubunganpj
            $this->hubunganpj->setDbValueDef($rsnew, $this->hubunganpj->CurrentValue, null, $this->hubunganpj->ReadOnly);

            // biaya_reg
            $this->biaya_reg->setDbValueDef($rsnew, $this->biaya_reg->CurrentValue, null, $this->biaya_reg->ReadOnly);

            // stts
            $this->stts->setDbValueDef($rsnew, $this->stts->CurrentValue, null, $this->stts->ReadOnly);

            // stts_daftar
            $this->stts_daftar->setDbValueDef($rsnew, $this->stts_daftar->CurrentValue, null, $this->stts_daftar->ReadOnly);

            // status_lanjut
            $this->status_lanjut->setDbValueDef($rsnew, $this->status_lanjut->CurrentValue, null, $this->status_lanjut->ReadOnly);

            // kd_pj
            $this->kd_pj->setDbValueDef($rsnew, $this->kd_pj->CurrentValue, null, $this->kd_pj->ReadOnly);

            // umurdaftar
            $this->umurdaftar->setDbValueDef($rsnew, $this->umurdaftar->CurrentValue, null, $this->umurdaftar->ReadOnly);

            // sttsumur
            $this->sttsumur->setDbValueDef($rsnew, $this->sttsumur->CurrentValue, null, $this->sttsumur->ReadOnly);

            // status_bayar
            $this->status_bayar->setDbValueDef($rsnew, $this->status_bayar->CurrentValue, null, $this->status_bayar->ReadOnly);

            // status_poli
            $this->status_poli->setDbValueDef($rsnew, $this->status_poli->CurrentValue, null, $this->status_poli->ReadOnly);

            // id_reg
            $this->id_reg->setDbValueDef($rsnew, $this->id_reg->CurrentValue, 0, $this->id_reg->ReadOnly);
