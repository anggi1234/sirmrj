...
class CpptEdit extends Cppt
{
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("id_cppt") ?? Key(0) ?? Route(2)) !== null) {
                $this->id_cppt->setQueryStringValue($keyValue);
                $this->id_cppt->setOldValue($this->id_cppt->QueryStringValue);
            } elseif (Post("id_cppt") !== null) {
                $this->id_cppt->setFormValue(Post("id_cppt"));
                $this->id_cppt->setOldValue($this->id_cppt->FormValue);
            } else {
