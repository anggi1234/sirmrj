 ...
 * Page class
 */
class PoliklinikEdit extends Poliklinik
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PoliklinikEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("kd_poli") ?? Key(0) ?? Route(2)) !== null) {
                $this->kd_poli->setQueryStringValue($keyValue);
                $this->kd_poli->setOldValue($this->kd_poli->QueryStringValue);
            } elseif (Post("kd_poli") !== null) {
                $this->kd_poli->setFormValue(Post("kd_poli"));
                $this->kd_poli->setOldValue($this->kd_poli->FormValue);
            } else {
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // kd_poli
            $this->kd_poli->setDbValueDef($rsnew, $this->kd_poli->CurrentValue, "", $this->kd_poli->ReadOnly);

            // nm_poli
            $this->nm_poli->setDbValueDef($rsnew, $this->nm_poli->CurrentValue, null, $this->nm_poli->ReadOnly);

            // registrasi
            $this->registrasi->setDbValueDef($rsnew, $this->registrasi->CurrentValue, 0, $this->registrasi->ReadOnly);

            // registrasilama
            $this->registrasilama->setDbValueDef($rsnew, $this->registrasilama->CurrentValue, 0, $this->registrasilama->ReadOnly);
