 ...
 * Page class
 */
class ResepDokterGrid extends ResepDokter
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ResepDokterGrid";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fresep_doktergrid";
    ...
    {
        $this->jml->FormValue = ""; // Clear form value
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->id_resep_dokter->CurrentValue;
    ...
    public function emptyRow()
    {
        ...
        if ($CurrentForm->hasValue("x_kode_brng") && $CurrentForm->hasValue("o_kode_brng") && $this->kode_brng->CurrentValue != $this->kode_brng->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_jml") && $CurrentForm->hasValue("o_jml") && $this->jml->CurrentValue != $this->jml->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_aturan_pakai") && $CurrentForm->hasValue("o_aturan_pakai") && $this->aturan_pakai->CurrentValue != $this->aturan_pakai->OldValue) {
            return false;
        }
        return true;
    }
    ...
    {
        $this->id_resep_dokter->clearErrorMessage();
        $this->no_reg->clearErrorMessage();
        $this->kode_brng->clearErrorMessage();
        $this->jml->clearErrorMessage();
        $this->aturan_pakai->clearErrorMessage();
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->id_resep_dokter->OldValue = $this->id_resep_dokter->CurrentValue;
        $this->no_reg->CurrentValue = null;
        $this->no_reg->OldValue = $this->no_reg->CurrentValue;
        $this->kode_brng->CurrentValue = null;
        $this->kode_brng->OldValue = $this->kode_brng->CurrentValue;
        $this->jml->CurrentValue = null;
        $this->jml->OldValue = $this->jml->CurrentValue;
        $this->aturan_pakai->CurrentValue = null;
        $this->aturan_pakai->OldValue = $this->aturan_pakai->CurrentValue;
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->id_resep_dokter->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
        ...
        if ($CurrentForm->hasValue("o_kode_brng")) {
            $this->kode_brng->setOldValue($CurrentForm->getValue("o_kode_brng"));
        }
        ...
        if ($CurrentForm->hasValue("o_jml")) {
            $this->jml->setOldValue($CurrentForm->getValue("o_jml"));
        }
        ...
        if ($CurrentForm->hasValue("o_aturan_pakai")) {
            $this->aturan_pakai->setOldValue($CurrentForm->getValue("o_aturan_pakai"));
        }
    ...
    protected function newRow()
    {
        ...
        $row = [];
        $row['id_resep_dokter'] = $this->id_resep_dokter->CurrentValue;
        $row['no_reg'] = $this->no_reg->CurrentValue;
        $row['kode_brng'] = $this->kode_brng->CurrentValue;
        $row['jml'] = $this->jml->CurrentValue;
        $row['aturan_pakai'] = $this->aturan_pakai->CurrentValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if (strval($this->jml->EditValue) != "" && is_numeric($this->jml->EditValue)) {
                ...
                $this->jml->OldValue = $this->jml->EditValue;
            }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // Check referential integrity for master table 'resep_dokter'
        $validMasterRecord = true;
        $masterFilter = $this->sqlMasterFilter_vrajal();
        ...
        // kode_brng
        $this->kode_brng->setDbValueDef($rsnew, $this->kode_brng->CurrentValue, null, false);

        // jml
        $this->jml->setDbValueDef($rsnew, $this->jml->CurrentValue, null, false);

        // aturan_pakai
        $this->aturan_pakai->setDbValueDef($rsnew, $this->aturan_pakai->CurrentValue, null, false);
