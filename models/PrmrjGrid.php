...
class PrmrjGrid extends Prmrj
{
    ...
    // Page object name
    public $PageObjName = "PrmrjGrid";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fprmrjgrid";
    ...
    {
        global $CurrentForm;
        if ($CurrentForm->hasValue("x_no_rkm_medis") && $CurrentForm->hasValue("o_no_rkm_medis") && $this->no_rkm_medis->CurrentValue != $this->no_rkm_medis->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_kd_poli") && $CurrentForm->hasValue("o_kd_poli") && $this->kd_poli->CurrentValue != $this->kd_poli->OldValue) {
            return false;
        }
        ...
        if ($CurrentForm->hasValue("x_kd_dokter") && $CurrentForm->hasValue("o_kd_dokter") && $this->kd_dokter->CurrentValue != $this->kd_dokter->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_kd_icd9") && $CurrentForm->hasValue("o_kd_icd9") && $this->kd_icd9->CurrentValue != $this->kd_icd9->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_cetak") && $CurrentForm->hasValue("o_cetak") && $this->cetak->CurrentValue != $this->cetak->OldValue) {
            return false;
        }
        return true;
    }
    ...
    {
        $this->no_rkm_medis->clearErrorMessage();
        $this->tgl_registrasi->clearErrorMessage();
        $this->kd_poli->clearErrorMessage();
        $this->kd_penyakit->clearErrorMessage();
        $this->alergi->clearErrorMessage();
        $this->kd_dokter->clearErrorMessage();
        $this->kd_icd9->clearErrorMessage();
        $this->cetak->clearErrorMessage();
    }
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_no_rkm_medis")) {
            $this->no_rkm_medis->setOldValue($CurrentForm->getValue("o_no_rkm_medis"));
        }
        ...
        if (!$this->tgl_registrasi->IsDetailKey) {
            ...
            $this->tgl_registrasi->CurrentValue = UnFormatDateTime($this->tgl_registrasi->CurrentValue, 6);
        }
        if ($CurrentForm->hasValue("o_tgl_registrasi")) {
            $this->tgl_registrasi->setOldValue($CurrentForm->getValue("o_tgl_registrasi"));
        }
        ...
        if ($CurrentForm->hasValue("o_kd_poli")) {
            $this->kd_poli->setOldValue($CurrentForm->getValue("o_kd_poli"));
        }
        ...
        if ($CurrentForm->hasValue("o_kd_dokter")) {
            $this->kd_dokter->setOldValue($CurrentForm->getValue("o_kd_dokter"));
        }
        ...
        if ($CurrentForm->hasValue("o_cetak")) {
            $this->cetak->setOldValue($CurrentForm->getValue("o_cetak"));
        }
    }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            $keyValue = $rsnew['no_rkm_medis'] ?? $rsold['no_rkm_medis'];
            if (strval($keyValue) != "") {
                $masterFilter = str_replace("@no_rkm_medis@", AdjustSql($keyValue), $masterFilter);
            } else {
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->getCurrentMasterTable() == "vrajal") {
            $this->no_rkm_medis->CurrentValue = $this->no_rkm_medis->getSessionValue();
        }

        // Check referential integrity for master table 'prmrj'
        $validMasterRecord = true;
        $masterFilter = $this->sqlMasterFilter_vrajal();
        if (strval($this->no_rkm_medis->CurrentValue) != "") {
            $masterFilter = str_replace("@no_rkm_medis@", AdjustSql($this->no_rkm_medis->CurrentValue, "DB"), $masterFilter);
        } else {
            $validMasterRecord = false;
        }
        if ($validMasterRecord) {
