 ...
 * Page class
 */
class ResepDokterView extends ResepDokter
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ResepDokterView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("id_resep_dokter") ?? Route("id_resep_dokter")) !== null) {
            $this->RecKey["id_resep_dokter"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("id_resep_dokter") ?? Route("id_resep_dokter")) !== null) {
                ...
                $this->RecKey["id_resep_dokter"] = $this->id_resep_dokter->QueryStringValue;
            } elseif (Post("id_resep_dokter") !== null) {
                $this->id_resep_dokter->setFormValue(Post("id_resep_dokter"));
                $this->RecKey["id_resep_dokter"] = $this->id_resep_dokter->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->id_resep_dokter->setQueryStringValue($keyValue);
                $this->RecKey["id_resep_dokter"] = $this->id_resep_dokter->QueryStringValue;
            } else {
                $returnUrl = "ResepDokterList"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "ResepDokterList"; // No matching record, return to list
                    }
                    break;
            }
        } else {
            $returnUrl = "ResepDokterList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
