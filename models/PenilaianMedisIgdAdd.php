 ...
 * Page class
 */
class PenilaianMedisIgdAdd extends PenilaianMedisIgd
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PenilaianMedisIgdAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("PenilaianMedisIgdList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "PenilaianMedisIgdList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "PenilaianMedisIgdView") {
    ...
    {
        $this->id_penilaian_medis_igd->CurrentValue = null;
        $this->id_penilaian_medis_igd->OldValue = $this->id_penilaian_medis_igd->CurrentValue;
        ...
        $this->mata->CurrentValue = null;
        $this->mata->OldValue = $this->mata->CurrentValue;
        $this->gigi->CurrentValue = null;
        $this->gigi->OldValue = $this->gigi->CurrentValue;
        $this->leher->CurrentValue = null;
        $this->leher->OldValue = $this->leher->CurrentValue;
        ...
        $this->ekg->CurrentValue = null;
        $this->ekg->OldValue = $this->ekg->CurrentValue;
        $this->rad->CurrentValue = null;
        $this->rad->OldValue = $this->rad->CurrentValue;
        $this->lab->CurrentValue = null;
        $this->lab->OldValue = $this->lab->CurrentValue;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'leher' first before field var 'x_leher'
        $val = $CurrentForm->hasValue("leher") ? $CurrentForm->getValue("leher") : $CurrentForm->getValue("x_leher");
        if (!$this->leher->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->leher->Visible = false; // Disable update for API request
            } else {
                $this->leher->setFormValue($val);
            }
        }
        ...
        // Check field name 'ekg' first before field var 'x_ekg'
        $val = $CurrentForm->hasValue("ekg") ? $CurrentForm->getValue("ekg") : $CurrentForm->getValue("x_ekg");
        if (!$this->ekg->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->ekg->Visible = false; // Disable update for API request
            } else {
                $this->ekg->setFormValue($val);
            }
        }

        // Check field name 'rad' first before field var 'x_rad'
        $val = $CurrentForm->hasValue("rad") ? $CurrentForm->getValue("rad") : $CurrentForm->getValue("x_rad");
        if (!$this->rad->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->rad->Visible = false; // Disable update for API request
            } else {
                $this->rad->setFormValue($val);
            }
        }

        // Check field name 'lab' first before field var 'x_lab'
        $val = $CurrentForm->hasValue("lab") ? $CurrentForm->getValue("lab") : $CurrentForm->getValue("x_lab");
        if (!$this->lab->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->lab->Visible = false; // Disable update for API request
            } else {
                $this->lab->setFormValue($val);
            }
        }
        ...
        // Check field name 'id_penilaian_medis_igd' first before field var 'x_id_penilaian_medis_igd'
        $val = $CurrentForm->hasValue("id_penilaian_medis_igd") ? $CurrentForm->getValue("id_penilaian_medis_igd") : $CurrentForm->getValue("x_id_penilaian_medis_igd");
    }
    ...
    public function restoreFormValues()
    {
        ...
        $this->leher->CurrentValue = $this->leher->FormValue;
        ...
        $this->ekg->CurrentValue = $this->ekg->FormValue;
        $this->rad->CurrentValue = $this->rad->FormValue;
        $this->lab->CurrentValue = $this->lab->FormValue;
    ...
    protected function newRow()
    {
        ...
        $row = [];
        $row['id_penilaian_medis_igd'] = $this->id_penilaian_medis_igd->CurrentValue;
        ...
        $row['mata'] = $this->mata->CurrentValue;
        $row['gigi'] = $this->gigi->CurrentValue;
        $row['leher'] = $this->leher->CurrentValue;
        ...
        $row['ekg'] = $this->ekg->CurrentValue;
        $row['rad'] = $this->rad->CurrentValue;
        $row['lab'] = $this->lab->CurrentValue;
    ...
    protected function validateForm()
    {
        ...
        if ($this->leher->Required) {
            if ($this->leher->FormValue == "") {
                $this->leher->addErrorMessage(str_replace("%s", $this->leher->caption(), $this->leher->RequiredErrorMessage));
            }
        }
        ...
        if ($this->ekg->Required) {
            if (!$this->ekg->IsDetailKey && EmptyValue($this->ekg->FormValue)) {
                $this->ekg->addErrorMessage(str_replace("%s", $this->ekg->caption(), $this->ekg->RequiredErrorMessage));
            }
        }
        if ($this->rad->Required) {
            if (!$this->rad->IsDetailKey && EmptyValue($this->rad->FormValue)) {
                $this->rad->addErrorMessage(str_replace("%s", $this->rad->caption(), $this->rad->RequiredErrorMessage));
            }
        }
        if ($this->lab->Required) {
            if (!$this->lab->IsDetailKey && EmptyValue($this->lab->FormValue)) {
                $this->lab->addErrorMessage(str_replace("%s", $this->lab->caption(), $this->lab->RequiredErrorMessage));
            }
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // rps
        $this->rps->setDbValueDef($rsnew, $this->rps->CurrentValue, "", false);
        ...
        // spo
        $this->spo->setDbValueDef($rsnew, $this->spo->CurrentValue, "", false);
        ...
        // kepala
        $this->kepala->setDbValueDef($rsnew, $this->kepala->CurrentValue, "", false);

        // mata
        $this->mata->setDbValueDef($rsnew, $this->mata->CurrentValue, "", false);

        // gigi
        $this->gigi->setDbValueDef($rsnew, $this->gigi->CurrentValue, "", false);

        // leher
        $this->leher->setDbValueDef($rsnew, $this->leher->CurrentValue, "", false);

        // thoraks
        $this->thoraks->setDbValueDef($rsnew, $this->thoraks->CurrentValue, "", false);

        // abdomen
        $this->abdomen->setDbValueDef($rsnew, $this->abdomen->CurrentValue, "", false);

        // genital
        $this->genital->setDbValueDef($rsnew, $this->genital->CurrentValue, "", false);

        // ekstremitas
        $this->ekstremitas->setDbValueDef($rsnew, $this->ekstremitas->CurrentValue, "", false);

        // ket_fisik
        $this->ket_fisik->setDbValueDef($rsnew, $this->ket_fisik->CurrentValue, "", false);

        // ket_lokalis
        $this->ket_lokalis->setDbValueDef($rsnew, $this->ket_lokalis->CurrentValue, "", false);

        // ekg
        $this->ekg->setDbValueDef($rsnew, $this->ekg->CurrentValue, "", false);

        // rad
        $this->rad->setDbValueDef($rsnew, $this->rad->CurrentValue, "", false);

        // lab
        $this->lab->setDbValueDef($rsnew, $this->lab->CurrentValue, "", false);

        // diagnosis
        $this->diagnosis->setDbValueDef($rsnew, $this->diagnosis->CurrentValue, "", false);

        // tata
        $this->tata->setDbValueDef($rsnew, $this->tata->CurrentValue, "", false);
