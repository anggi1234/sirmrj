...
class CpptGrid extends Cppt
{
    ...
    // Page object name
    public $PageObjName = "CpptGrid";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fcpptgrid";
    ...
    public function emptyRow()
    {
        ...
        if ($CurrentForm->hasValue("x_hasil_soap") && $CurrentForm->hasValue("o_hasil_soap") && $this->hasil_soap->CurrentValue != $this->hasil_soap->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_instruksi") && $CurrentForm->hasValue("o_instruksi") && $this->instruksi->CurrentValue != $this->instruksi->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_verifikasi") && $CurrentForm->hasValue("o_verifikasi") && $this->verifikasi->CurrentValue != $this->verifikasi->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_tanggal_input") && $CurrentForm->hasValue("o_tanggal_input") && $this->tanggal_input->CurrentValue != $this->tanggal_input->OldValue) {
            return false;
        }
        return true;
    }
    ...
    public function resetFormError()
    {
        ...
        $this->hasil_soap->clearErrorMessage();
        $this->instruksi->clearErrorMessage();
        $this->verifikasi->clearErrorMessage();
    ...
    {
        $this->id_cppt->CurrentValue = null;
        $this->id_cppt->OldValue = $this->id_cppt->CurrentValue;
        ...
        $this->hasil_soap->CurrentValue = null;
        $this->hasil_soap->OldValue = $this->hasil_soap->CurrentValue;
        $this->instruksi->CurrentValue = null;
        $this->instruksi->OldValue = $this->instruksi->CurrentValue;
        $this->verifikasi->CurrentValue = null;
        $this->verifikasi->OldValue = $this->verifikasi->CurrentValue;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'hasil_soap' first before field var 'x_hasil_soap'
        $val = $CurrentForm->hasValue("hasil_soap") ? $CurrentForm->getValue("hasil_soap") : $CurrentForm->getValue("x_hasil_soap");
        if (!$this->hasil_soap->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->hasil_soap->Visible = false; // Disable update for API request
            } else {
                $this->hasil_soap->setFormValue($val);
            }
        }
        if ($CurrentForm->hasValue("o_hasil_soap")) {
            $this->hasil_soap->setOldValue($CurrentForm->getValue("o_hasil_soap"));
        }
        ...
        if ($CurrentForm->hasValue("o_instruksi")) {
            $this->instruksi->setOldValue($CurrentForm->getValue("o_instruksi"));
        }

        // Check field name 'verifikasi' first before field var 'x_verifikasi'
        $val = $CurrentForm->hasValue("verifikasi") ? $CurrentForm->getValue("verifikasi") : $CurrentForm->getValue("x_verifikasi");
        if (!$this->verifikasi->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->verifikasi->Visible = false; // Disable update for API request
            } else {
                $this->verifikasi->setFormValue($val);
            }
        }
        if ($CurrentForm->hasValue("o_verifikasi")) {
            $this->verifikasi->setOldValue($CurrentForm->getValue("o_verifikasi"));
        }
        ...
        if (!$this->tanggal_input->IsDetailKey) {
            ...
            $this->tanggal_input->CurrentValue = UnFormatDateTime($this->tanggal_input->CurrentValue, 0);
        }
        if ($CurrentForm->hasValue("o_tanggal_input")) {
            $this->tanggal_input->setOldValue($CurrentForm->getValue("o_tanggal_input"));
        }

        // Check field name 'id_cppt' first before field var 'x_id_cppt'
        $val = $CurrentForm->hasValue("id_cppt") ? $CurrentForm->getValue("id_cppt") : $CurrentForm->getValue("x_id_cppt");
        if (!$this->id_cppt->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
            $this->id_cppt->setFormValue($val);
        }
    }
    ...
    public function restoreFormValues()
    {
        ...
        if (!$this->isGridAdd() && !$this->isAdd()) {
            $this->id_cppt->CurrentValue = $this->id_cppt->FormValue;
        }
        $this->no_reg->CurrentValue = $this->no_reg->FormValue;
        $this->profesi->CurrentValue = $this->profesi->FormValue;
        $this->hasil_soap->CurrentValue = $this->hasil_soap->FormValue;
        $this->instruksi->CurrentValue = $this->instruksi->FormValue;
        $this->verifikasi->CurrentValue = $this->verifikasi->FormValue;
    ...
    protected function newRow()
    {
        ...
        $row = [];
        $row['id_cppt'] = $this->id_cppt->CurrentValue;
        $row['no_reg'] = $this->no_reg->CurrentValue;
        $row['profesi'] = $this->profesi->CurrentValue;
        $row['hasil_soap'] = $this->hasil_soap->CurrentValue;
        $row['instruksi'] = $this->instruksi->CurrentValue;
        $row['verifikasi'] = $this->verifikasi->CurrentValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->hasil_soap->EditValue = HtmlEncode($this->hasil_soap->CurrentValue);
            ...
            $this->tanggal_input->EditValue = HtmlEncode(FormatDateTime($this->tanggal_input->CurrentValue, 8));
    ...
    protected function validateForm()
    {
        ...
        if ($this->profesi->Required) {
            if ($this->profesi->FormValue == "") {
                $this->profesi->addErrorMessage(str_replace("%s", $this->profesi->caption(), $this->profesi->RequiredErrorMessage));
            }
        }
        if ($this->hasil_soap->Required) {
            if (!$this->hasil_soap->IsDetailKey && EmptyValue($this->hasil_soap->FormValue)) {
                $this->hasil_soap->addErrorMessage(str_replace("%s", $this->hasil_soap->caption(), $this->hasil_soap->RequiredErrorMessage));
            }
        }
        ...
        if ($this->verifikasi->Required) {
            if ($this->verifikasi->FormValue == "") {
                $this->verifikasi->addErrorMessage(str_replace("%s", $this->verifikasi->caption(), $this->verifikasi->RequiredErrorMessage));
            }
        }
        ...
        if (!CheckDate($this->tanggal_input->FormValue)) {
            $this->tanggal_input->addErrorMessage($this->tanggal_input->getErrorMessage(false));
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // hasil_soap
            $this->hasil_soap->setDbValueDef($rsnew, $this->hasil_soap->CurrentValue, null, $this->hasil_soap->ReadOnly);

            // instruksi
            $this->instruksi->setDbValueDef($rsnew, $this->instruksi->CurrentValue, null, $this->instruksi->ReadOnly);

            // verifikasi
            $this->verifikasi->setDbValueDef($rsnew, $this->verifikasi->CurrentValue, null, $this->verifikasi->ReadOnly);

            // tanggal_input
            $this->tanggal_input->setDbValueDef($rsnew, UnFormatDateTime($this->tanggal_input->CurrentValue, 0), null, $this->tanggal_input->ReadOnly);
    ...
    protected function addRow($rsold = null)
    {
        ...
        // Check referential integrity for master table 'cppt'
        $validMasterRecord = true;
        $masterFilter = $this->sqlMasterFilter_vrajal();
        ...
        // hasil_soap
        $this->hasil_soap->setDbValueDef($rsnew, $this->hasil_soap->CurrentValue, null, false);

        // instruksi
        $this->instruksi->setDbValueDef($rsnew, $this->instruksi->CurrentValue, null, false);

        // verifikasi
        $this->verifikasi->setDbValueDef($rsnew, $this->verifikasi->CurrentValue, null, false);

        // tanggal_input
        $this->tanggal_input->setDbValueDef($rsnew, UnFormatDateTime($this->tanggal_input->CurrentValue, 0), null, false);
