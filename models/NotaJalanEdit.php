 ...
 * Page class
 */
class NotaJalanEdit extends NotaJalan
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "NotaJalanEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("no_rawat") ?? Key(0) ?? Route(2)) !== null) {
                $this->no_rawat->setQueryStringValue($keyValue);
                $this->no_rawat->setOldValue($this->no_rawat->QueryStringValue);
            } elseif (Post("no_rawat") !== null) {
                $this->no_rawat->setFormValue(Post("no_rawat"));
                $this->no_rawat->setOldValue($this->no_rawat->FormValue);
            } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$loaded) { // Load record based on key
                    ...
                    $this->terminate("NotaJalanList"); // No matching record, return to list
                    return;
                }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "NotaJalanList") {
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'no_nota' first before field var 'x_no_nota'
        $val = $CurrentForm->hasValue("no_nota") ? $CurrentForm->getValue("no_nota") : $CurrentForm->getValue("x_no_nota");
        if (!$this->no_nota->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->no_nota->Visible = false; // Disable update for API request
            } else {
                $this->no_nota->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->no_nota->CurrentValue = $this->no_nota->FormValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // no_nota
            $this->no_nota->EditAttrs["class"] = "form-control";
            $this->no_nota->EditCustomAttributes = "";
            if (!$this->no_nota->Raw) {
                $this->no_nota->CurrentValue = HtmlDecode($this->no_nota->CurrentValue);
            }
            $this->no_nota->EditValue = HtmlEncode($this->no_nota->CurrentValue);
            $this->no_nota->PlaceHolder = RemoveHtml($this->no_nota->caption());
            ...
            $this->jam->EditValue = HtmlEncode(FormatDateTime($this->jam->CurrentValue, 4));
    ...
    protected function validateForm()
    {
        ...
        if ($this->no_nota->Required) {
            if (!$this->no_nota->IsDetailKey && EmptyValue($this->no_nota->FormValue)) {
                $this->no_nota->addErrorMessage(str_replace("%s", $this->no_nota->caption(), $this->no_nota->RequiredErrorMessage));
            }
        }
        ...
        if (!CheckTime($this->jam->FormValue)) {
            $this->jam->addErrorMessage($this->jam->getErrorMessage(false));
        }
    ...
    protected function editRow()
    {
        ...
        $conn = $this->getConnection();
        if ($this->no_nota->CurrentValue != "") { // Check field with unique index
            $filterChk = "(`no_nota` = '" . AdjustSql($this->no_nota->CurrentValue, $this->Dbid) . "')";
            ...
            if ($rsChk->fetch()) {
                $idxErrMsg = str_replace("%f", $this->no_nota->caption(), $Language->phrase("DupIndex"));
                $idxErrMsg = str_replace("%v", $this->no_nota->CurrentValue, $idxErrMsg);
        ...
        if (!$rsold) {
            ...
            // no_nota
            $this->no_nota->setDbValueDef($rsnew, $this->no_nota->CurrentValue, null, $this->no_nota->ReadOnly);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("NotaJalanList"), "", $this->TableVar, true);
        $pageId = "edit";
