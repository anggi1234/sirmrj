 ...
 * Page class
 */
class PenilaianPsikologiEdit extends PenilaianPsikologi
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PenilaianPsikologiEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("id_penilaian_psikologi") ?? Key(0) ?? Route(2)) !== null) {
                $this->id_penilaian_psikologi->setQueryStringValue($keyValue);
                $this->id_penilaian_psikologi->setOldValue($this->id_penilaian_psikologi->QueryStringValue);
            } elseif (Post("id_penilaian_psikologi") !== null) {
                $this->id_penilaian_psikologi->setFormValue(Post("id_penilaian_psikologi"));
                $this->id_penilaian_psikologi->setOldValue($this->id_penilaian_psikologi->FormValue);
            } else {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->id_penilaian_psikologi->IsDetailKey) {
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // ket_anamnesis
            $this->ket_anamnesis->setDbValueDef($rsnew, $this->ket_anamnesis->CurrentValue, "", $this->ket_anamnesis->ReadOnly);
            ...
            // ciri_menyolok
            $this->ciri_menyolok->setDbValueDef($rsnew, $this->ciri_menyolok->CurrentValue, "", $this->ciri_menyolok->ReadOnly);

            // hasil_psikotes
            $this->hasil_psikotes->setDbValueDef($rsnew, $this->hasil_psikotes->CurrentValue, "", $this->hasil_psikotes->ReadOnly);

            // kepribadian
            $this->kepribadian->setDbValueDef($rsnew, $this->kepribadian->CurrentValue, "", $this->kepribadian->ReadOnly);

            // psikodinamika
            $this->psikodinamika->setDbValueDef($rsnew, $this->psikodinamika->CurrentValue, "", $this->psikodinamika->ReadOnly);

            // kesimpulan_psikolog
            $this->kesimpulan_psikolog->setDbValueDef($rsnew, $this->kesimpulan_psikolog->CurrentValue, "", $this->kesimpulan_psikolog->ReadOnly);
