 ...
 * Page class
 */
class KonsulView extends Konsul
{
    use MessagesTrait;
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("id_konsul") ?? Route("id_konsul")) !== null) {
            $this->RecKey["id_konsul"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("id_konsul") ?? Route("id_konsul")) !== null) {
                ...
                $this->RecKey["id_konsul"] = $this->id_konsul->QueryStringValue;
            } elseif (Post("id_konsul") !== null) {
                $this->id_konsul->setFormValue(Post("id_konsul"));
                $this->RecKey["id_konsul"] = $this->id_konsul->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->id_konsul->setQueryStringValue($keyValue);
                $this->RecKey["id_konsul"] = $this->id_konsul->QueryStringValue;
            } else {
                $returnUrl = "KonsulList"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "KonsulList"; // No matching record, return to list
                    }
                    break;
            }
        } else {
            $returnUrl = "KonsulList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
