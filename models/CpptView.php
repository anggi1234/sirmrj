    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("id_cppt") ?? Route("id_cppt")) !== null) {
                ...
                $this->RecKey["id_cppt"] = $this->id_cppt->QueryStringValue;
            } elseif (Post("id_cppt") !== null) {
                $this->id_cppt->setFormValue(Post("id_cppt"));
                $this->RecKey["id_cppt"] = $this->id_cppt->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->id_cppt->setQueryStringValue($keyValue);
                $this->RecKey["id_cppt"] = $this->id_cppt->QueryStringValue;
            } else {
                $returnUrl = "CpptList"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "CpptList"; // No matching record, return to list
                    }
                    break;
            }
        } else {
            $returnUrl = "CpptList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
