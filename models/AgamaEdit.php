...
class AgamaEdit extends Agama
{
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("id_agama") ?? Key(0) ?? Route(2)) !== null) {
                $this->id_agama->setQueryStringValue($keyValue);
                $this->id_agama->setOldValue($this->id_agama->QueryStringValue);
            } elseif (Post("id_agama") !== null) {
                $this->id_agama->setFormValue(Post("id_agama"));
                $this->id_agama->setOldValue($this->id_agama->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("id_agama") ?? Route("id_agama")) !== null) {
                    $this->id_agama->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$loaded) { // Load record based on key
                    ...
                    $this->terminate("AgamaList"); // No matching record, return to list
                    return;
                }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "AgamaList") {
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'id_agama' first before field var 'x_id_agama'
        $val = $CurrentForm->hasValue("id_agama") ? $CurrentForm->getValue("id_agama") : $CurrentForm->getValue("x_id_agama");
        if (!$this->id_agama->IsDetailKey) {
            $this->id_agama->setFormValue($val);
        }
    ...
    {
        global $CurrentForm;
        $this->id_agama->CurrentValue = $this->id_agama->FormValue;
    ...
    protected function validateForm()
    {
        ...
        if ($this->id_agama->Required) {
            if (!$this->id_agama->IsDetailKey && EmptyValue($this->id_agama->FormValue)) {
                $this->id_agama->addErrorMessage(str_replace("%s", $this->id_agama->caption(), $this->id_agama->RequiredErrorMessage));
            }
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("AgamaList"), "", $this->TableVar, true);
        $pageId = "edit";
